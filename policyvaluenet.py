...
class TransformerNet(nn.Module):
    def __init__(self, board_width, board_height, d_model=128, nhead=8, num_layers=2):
        ...
        self.value_head_fc2 = nn.Linear(128, 2)

    def forward(self, state_input):
        x = self.embedding(state_input)  # Shape: (batch_size, board_height, d_model)
        # x = x.permute(1, 0, 2)  # Transformer expects (seq_len, batch_size, d_model)
        x = self.transformer(x)
        # x = x.permute(1, 0, 2)  # Back to (batch_size, board_height, d_model)

        # Policy head
        x_policy = x.flatten(1)  # Flatten all features
        # todo: replace by mask softmax
...
class PolicyValueNet:
    ...
    def policy_value_fn(self, board: Seqenv):
        """
        input: board
